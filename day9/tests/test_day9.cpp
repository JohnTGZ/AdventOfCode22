...
TEST_F(GridMapTest, MoveTailIndividual) {
    ...
    gridmap_->move_all_cells( "L", 3);
    ASSERT_EQ(gridmap_->get_cell("C1")->position, Position2D(2, 0));

    gridmap_->move_all_cells( "D", 1);
    ASSERT_EQ(gridmap_->get_cell("C1")->position, Position2D(2, 0));
}
...
TEST_F(GridMapTestPart2, MoveTailIndividual) {
    ...
    ASSERT_EQ(gridmap_->get_cell("C6")->position, Position2D(14, 12));
    ASSERT_EQ(gridmap_->get_cell("C7")->position, Position2D(13, 13));
    ASSERT_EQ(gridmap_->get_cell("C8")->position, Position2D(12, 14));
    ASSERT_EQ(gridmap_->get_cell("C9")->position, Position2D(11, 15));

    gridmap_->move_all_cells("L", 8);
    ASSERT_EQ(gridmap_->get_cell("C0")->position, Position2D(8, 7));
    ASSERT_EQ(gridmap_->get_cell("C1")->position, Position2D(9, 7));
    ASSERT_EQ(gridmap_->get_cell("C2")->position, Position2D(10, 7));
    ASSERT_EQ(gridmap_->get_cell("C3")->position, Position2D(11, 7));
    ASSERT_EQ(gridmap_->get_cell("C4")->position, Position2D(12, 7));
    ASSERT_EQ(gridmap_->get_cell("C5")->position, Position2D(12, 8));
    ASSERT_EQ(gridmap_->get_cell("C6")->position, Position2D(12, 9));
    ASSERT_EQ(gridmap_->get_cell("C7")->position, Position2D(12, 10));
    ASSERT_EQ(gridmap_->get_cell("C8")->position, Position2D(12, 11));
    ASSERT_EQ(gridmap_->get_cell("C9")->position, Position2D(12, 12));
}

TEST_F(GridMapTestPart2, NumVisitedPart2) {
    for (int i = 0; i < lines_.size(); i++){
        const auto inst = FileParser::split(lines_[i], ' ');
        gridmap_->move_all_cells( inst[0], std::stoi(inst[1].c_str()), true);
    }

    ASSERT_EQ(gridmap_->get_num_visited(), 36);
}