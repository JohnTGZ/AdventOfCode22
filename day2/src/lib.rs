...
mod tests {
    ...
    fn setup() -> RPS {
        ...
        RPS::build(&file_contents).expect("Error building a RPS (Rock,Papers,Scissors) object")
    }
...
impl RPS {
    pub fn build(file_contents: &FileContents) -> Option<RPS> {
        ...
        for (shape_name, meta_data) in shape_metadata_hash.iter() {
            println!(
                "{} : Score[{}], Wins[{}], Draws[{}] ",
                shape_name, meta_data.shape_score, meta_data.win, meta_data.draw
            );
        }
    ...
    pub fn get_total_score_part1(&self) -> u64 {
        ...
        for rps_move in &self.rps_moves {
            let match_score = self
                .get_score_part1(&rps_move[0], &rps_move[1])
                .unwrap_or_else(|| {
    ...
    pub fn get_total_score_part2(&self) -> u64 {
        ...
        for rps_move in &self.rps_moves {
            let match_score = self
                .get_score_part2(&rps_move[0], &rps_move[1])
                .unwrap_or_else(|| {
                    panic!("Unable to get score!");
                });
            total_score += u64::from(match_score);
