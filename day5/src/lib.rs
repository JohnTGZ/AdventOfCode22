...
mod tests {
    ...
    fn setup() -> (Instructions, Shipyard) {
        ...
        let file_contents_instructions =
            FileContents::build(test_input_filepath, split_delim, 5, -1).unwrap_or_else(|err| {
                panic!("Unable to parse file: {err}");
            });

        let file_contents_shipyard = FileContents::build(test_input_filepath, split_delim, 0, 3)
            .unwrap_or_else(|err| {
                panic!("Unable to parse file: {err}");
            });
        ...
        (
            ...
            Shipyard::build(&file_contents_shipyard, 3),
        )
    }
...
impl Instructions {
    pub fn build(file_contents: &FileContents) -> Instructions {
        ...
        let re = Regex::new(r"(\w+) ([0-9]+) (\w+) ([0-9]+) (\w+) ([0-9]+)")
